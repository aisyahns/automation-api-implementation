...
const schema = require('../../schema/User/DELETE_deletebyuserid.schema.js');
...
class pages {
    constructor() {
		...
		this.path = "/Account/v1/User/" // Set up the API path to the route endpoint
    }
    ...
    request(...args) {
		...
		const response = this.api.delete(this.path + args[0])
		.set("Authorization", "Bearer " + args[1])
